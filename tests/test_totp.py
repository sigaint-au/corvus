...
class TestTotp:
    ...
    def test_needs_challenge(self):
        ...
        with (
            patch.object(totp_svc, "is_enabled", return_value=False),
            patch.object(totp_svc, "enforce_global_admins", return_value=True),
        ):
            assert totp_svc.needs_challenge(uid, True) == "enroll"
            assert totp_svc.needs_challenge(uid, False) is None
        with (
            patch.object(totp_svc, "is_enabled", return_value=False),
            patch.object(totp_svc, "enforce_global_admins", return_value=False),
        ):
    ...
    def test_login_redirects_to_2fa(self):
        ...
        with (
            ...
            patch("auth.totp_svc.needs_challenge", return_value="verify"),
        ):
            r = client.post(
    ...
    def test_login_2fa_ok(self):
        ...
        with (
            patch("auth.totp_svc.verify_user_code", return_value=(True, "totp")),
            patch("auth.lockout.is_locked", return_value=False),
            patch("auth.lockout.clear_failures"),
            patch.object(authz, "is_account_disabled", return_value=False),
            patch("auth.user_sessions.create_session", return_value=None),
    ...
    def test_login_enroll_admin(self):
        ...
        with (
            ...
            patch.object(authz, "is_account_disabled", return_value=False),
            patch("auth.totp_svc.needs_challenge", return_value="enroll"),
            patch("auth.user_sessions.create_session", return_value=None),
        ):
            r = client.post(
                "/login",
