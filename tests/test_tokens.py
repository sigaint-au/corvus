...
class TestTokens:
    ...
    def test_create_token(self):
        ...
        with (
            ...
            r = self.client.post(
                f"/projects/{self.pid}/tokens", data={"name": "openshift"}, follow_redirects=False
            )
    ...
    def test_create_token_write_role(self):
        ...
        with (
            ...
            r = self.client.post(
                ...
                data={"name": "ci-writer", "role": "service-write"},
    ...
    def test_create_token_requires_expiry_when_policy_enabled(self):
        ...
        with (
            patch.object(db, "as_user", return_value=conn),
            patch.object(settings_svc, "token_expiry_policy", return_value=(True, 3650)),
        ):
            r = self.client.post(
                f"/projects/{self.pid}/tokens", data={"name": "openshift"}, follow_redirects=False
            )
    ...
    def test_create_token_uses_policy_max(self):
        ...
        with (
            patch.object(db, "as_user", return_value=conn),
            patch.object(settings_svc, "token_expiry_policy", return_value=(False, 30)),
        ):
            r = self.client.post(
                f"/projects/{self.pid}/tokens",
                data={"name": "openshift", "expires_days": "31"},
                follow_redirects=False,
            )
        assert r.status_code == 302
        inserts = [
    ...
    def test_delete_token(self):
        ...
        with patch.object(db, "as_user", return_value=conn):
            r = self.client.post(
                f"/projects/{self.pid}/tokens/{uuid4()}/delete", follow_redirects=False
            )
        assert r.status_code == 302
