...
class TestMailer:
    def test_smtp_configured_requires_host_and_from(self):
        ...
        assert not mailer.smtp_configured(
            {"smtp_enabled": "true", "smtp_host": "", "smtp_from_email": "a@b.c"}
        )
        assert not mailer.smtp_configured(
            {"smtp_enabled": "false", "smtp_host": "h", "smtp_from_email": "a@b.c"}
        )
        assert mailer.smtp_configured(
            {"smtp_enabled": "true", "smtp_host": "smtp.example.com", "smtp_from_email": "a@b.c"}
        )

    def test_login_alerts_need_smtp(self):
        from integrations import mailer

        assert not mailer.login_alerts_enabled(
            {
                "smtp_enabled": "true",
                "smtp_host": "h",
        ...
        assert mailer.login_alerts_enabled(
            {
                "smtp_enabled": "true",
                "smtp_host": "h",
                "smtp_from_email": "a@b.c",
    ...
    def test_forgot_password_sends_email(self):
        ...
        with (
            patch("auth.passwords.create_reset_token", return_value="tok123"),
            patch("integrations.mailer.smtp_configured", return_value=True),
            patch("integrations.mailer.send_password_reset", return_value=(True, "")) as send,
        ):
            r = client.post(
                "/forgot-password", data={"email": "user@ex.com"}, follow_redirects=False
            )
    ...
    def test_login_sends_alert_when_enabled(self):
        ...
        with (
            ...
            patch.object(lockout, "is_locked", return_value=False),
            patch.object(lockout, "clear_failures"),
            ...
            patch("integrations.mailer.login_alerts_enabled", return_value=True),
            patch("integrations.mailer.send_login_alert", return_value=(True, "")) as alert,
