...
class TestAuth:
    def setup_method(self, method=None):
        ...
        self._patchers = [
            ...
            patch.object(
                settings_svc, "get_settings", return_value={"registration_enabled": "true"}
            ),
            patch.object(
                nav,
                "branding",
                return_value={"app_name": "Sigaint", "brand_name": "Sigaint", "brand_tagline": ""},
            ),
            patch.object(
                nav,
                "classification",
                return_value={"enabled": False, "text": "", "color": "#677381", "fg": "#ffffff"},
            ),
    ...
    def test_login_bad_creds(self):
        ...
        with (
            ...
            patch.object(ldap_auth, "ldap_cfg", return_value={"ldap_enabled": "false"}),
            patch("auth.lockout.record_failure"),
    ...
    def test_login_locked(self):
        with (
            patch("auth.lockout.is_locked", return_value=True),
            patch.object(ldap_auth, "ldap_cfg", return_value={"ldap_enabled": "false"}),
        ):
    ...
    def test_login_ok(self):
        ...
        with (
            ...
            r = self.client.post(
                "/login", data={"email": "a@b.c", "password": "secret12"}, follow_redirects=False
            )
        assert r.status_code == 302
        assert "/teams" in r.location
        with self.client.session_transaction() as s:
    ...
    def test_select_team_blocks_open_redirect(self):
        with self.client.session_transaction() as s:
            s["user_id"] = str(uuid4())
        r = self.client.post(
            "/select-team", data={"team_id": "", "next": "//evil.com"}, follow_redirects=False
        )
        assert r.status_code == 302
    ...
    def test_login_ldap_ok(self):
        ...
        with (
            patch.object(db, "connect", return_value=conn),
            patch.object(ldap_auth, "ldap_cfg", return_value={"ldap_enabled": "true"}),
            patch.object(ldap_auth, "ldap_authenticate", return_value=ldap_user),
            patch.object(ldap_auth, "sync_ldap_user", return_value=synced),
            patch("auth.lockout.is_locked", return_value=False),
            patch("auth.lockout.clear_failures"),
            ...
            r = self.client.post(
                "/login", data={"email": "ldapuser", "password": "dir-pass"}, follow_redirects=False
            )
        assert r.status_code == 302
        assert "/teams" in r.location
        with self.client.session_transaction() as s:
    ...
    def test_register_short_password(self):
        with (
            ...
            r = self.client.post(
                "/register", data={"email": "a@b.c", "password": "short", "name": "A"}
            )
        assert r.status_code == 400
    ...
    def test_bootstrap_email_promotes_on_register(self):
        ...
        with (
            ...
            patch.object(settings_svc, "registration_enabled", return_value=True),
            patch.object(settings_svc, "setup_notice", return_value=None),
            patch("routes.auth.helpers.bootstrap_admin_email", return_value="admin@ex.com"),
            patch.object(authz, "is_global_admin", return_value=True),
            patch("auth.totp_svc.needs_challenge", return_value=None),
            patch("integrations.mailer.login_alerts_enabled", return_value=False),
        ):
    ...
    def test_login_hides_register_when_disabled(self):
        with (
            patch.object(ldap_auth, "ldap_cfg", return_value={"ldap_enabled": "false"}),
            patch.object(settings_svc, "registration_enabled", return_value=False),
            patch.object(settings_svc, "setup_notice", return_value=None),
        ):
            r = self.client.get("/login")
        assert r.status_code == 200
    ...
    def test_registration_disabled_without_bootstrap(self):
        with (
            patch.object(settings_svc, "has_global_admin", return_value=False),
            patch("core.settings_svc.bootstrap_admin_email", return_value=""),
            patch.object(
                settings_svc, "get_settings", return_value={"registration_enabled": "true"}
            ),
        ):
    ...
    def test_forgot_password_post_no_enumeration(self):
        with patch("auth.passwords.create_reset_token", return_value=None):
            r = self.client.post(
                "/forgot-password", data={"email": "nobody@ex.com"}, follow_redirects=False
            )
        assert r.status_code == 302
        assert "/login" in r.location
    ...
    def test_change_password_ok(self):
        ...
        with self.client.session_transaction() as s:
            ...
            s["sid"] = str(uuid4())
        with (
            patch("auth.passwords.change_password", return_value=(True, "")),
            patch("auth.user_sessions.revoke_other_sessions", return_value=2),
        ):
            r = self.client.post(
                "/profile/password",
                data={
    ...
    def test_reset_password_mismatch(self):
        r = self.client.post(
            "/reset-password/tok", data={"password": "newpass12", "password_confirm": "nope"}
        )
        assert r.status_code == 400
                ...
                return [
                    {
                        ...
                        "role_description": "Owns the team",
                        "grant_kind": "Direct",
                        "grant_subject": "You",
                    ...
                    },
                    {
                        "scope_kind": "project",
                        "scope_label": "API",
                        "role_name": "project-write",
                        "role_description": "Write access",
                        "grant_kind": "Group",
                        "grant_subject": "platform-ops",
    ...
    def test_profile_ok(self):
        ...
        with (
            patch.object(db, "connect_admin", return_value=admin_conn),
            patch.object(db, "as_user", return_value=user_conn),
            patch("auth.user_sessions.list_sessions", return_value=[]),
        ):
            r = self.client.get("/profile")
        assert r.status_code == 200
        ...
        with (
            ...
            patch.object(db, "as_user", return_value=user_conn),
            patch("auth.user_sessions.list_sessions", return_value=[]),
            patch("auth.pats.list_for_user", return_value=[]),
        ):
