...
class TestSettings:
    ...
    def test_settings_ok_for_global_admin(self):
        ...
        with (
            ...
            patch.object(settings_svc, "get_settings", return_value=settings),
            patch.object(
                settings_svc,
                "classification",
                return_value={
    ...
    def test_settings_health_tab(self):
        with self.client.session_transaction() as s:
            ...
            s["is_global_admin"] = True
        conn, _ = _conn(
            fetchone={"v": "PostgreSQL 16.1 on x86_64", "db": "secrets", "usr": "admin"},
            fetchall=[],
        )
        with (
            patch.object(db, "as_user", return_value=_conn(fetchall=[])[0]),
            ...
            patch.object(cache, "redis_client", return_value=None),
        ):
    ...
    def test_health_test_redis_post_redirects(self):
        ...
        with (
            ...
            r = self.client.post(
                "/settings", data={"action": "health_test_redis"}, follow_redirects=False
            )
        assert r.status_code == 302
    ...
    def test_save_classification(self):
        ...
        with (
            patch.object(authz, "is_global_admin", return_value=True),
            patch.object(settings_svc, "set_setting", side_effect=set_setting),
            patch.object(db, "as_user", return_value=_conn(fetchall=[])[0]),
        ):
            r = self.client.post(
                "/settings",
    ...
    def test_save_token_policy(self):
        ...
        with (
            ...
            r = self.client.post(
                ...
                data={
                    "action": "token_policy",
                    "require_pat_expiry": "1",
                    "max_pat_lifetime_days": "90",
                    "require_machine_token_expiry": "1",
    ...
    def test_save_smtp(self):
        ...
        with (
            ...
            patch.object(settings_svc, "set_setting", side_effect=set_setting),
            patch.object(db, "as_user", return_value=_conn(fetchall=[])[0]),
            patch.object(crypto, "encrypt", return_value="encrypted-pw"),
        ):
            r = self.client.post(
                "/settings",
                data={
    ...
    def test_smtp_test_action(self):
        ...
        with (
            patch.object(authz, "is_global_admin", return_value=True),
            patch.object(db, "as_user", return_value=_conn(fetchall=[])[0]),
            patch("integrations.mailer.send_test_email", return_value=(True, "")) as send,
        ):
            r = self.client.post(
                "/settings",
    ...
    def test_user_disable_action(self):
        ...
        with (
            ...
            patch.object(db, "as_user", return_value=conn),
            patch.object(db, "connect_admin", return_value=conn),
            patch("auth.user_sessions.revoke_all_sessions", return_value=2) as rev,
        ):
            r = self.client.post(
                "/settings",
    ...
    def test_user_cannot_disable_self(self):
        ...
        with (
            patch.object(authz, "is_global_admin", return_value=True),
            patch.object(db, "as_user", return_value=_conn()[0]),
        ):
            r = self.client.post(
                "/settings",
    ...
    def test_user_reset_password_action(self):
        ...
        with (
            ...
            patch.object(db, "as_user", return_value=conn),
            patch.object(db, "connect_admin", return_value=conn),
            patch("auth.passwords.create_reset_token_for_user", return_value=("tok123", "")),
            patch("integrations.mailer.smtp_configured", return_value=False),
            patch("auth.user_sessions.revoke_all_sessions", return_value=0),
        ):
            r = self.client.post(
                "/settings",
    ...
    def test_user_reset_2fa_action(self):
        ...
        with (
            ...
            patch.object(db, "as_user", return_value=conn),
            patch.object(db, "connect_admin", return_value=conn),
            patch("auth.totp_svc.is_enabled", return_value=True),
            patch("auth.totp_svc.disable") as dis,
            patch("auth.user_sessions.revoke_all_sessions", return_value=1),
        ):
            r = self.client.post(
                "/settings",
