...
class TestSecrets:
    ...
    def test_project_access_tab(self):
        ...
        with patch.object(
            db,
            "as_user",
            return_value=self._project_conn(tab="requests", access_requests=reqs, pending_count=1),
        ):
    ...
    def test_project_audit_tab(self):
        ...
        with patch.object(
            db, "as_user", return_value=self._project_conn(tab="audit", audit_log=audit_rows)
        ):
    ...
    def test_project_settings_tab_shows_members_and_delete_for_owner(self):
        with patch.object(
            db, "as_user", return_value=self._project_conn(tab="settings", team_role="team-owner")
        ):
    ...
    def test_project_settings_hidden_for_writer_without_admin(self):
        ...
        with patch.object(
            ...
            return_value=self._project_conn(
                team_role="team-member", can_write=True, can_admin=False
            ),
        ):
            r = self.client.get(f"/projects/{self.pid}")
        assert r.status_code == 200
    ...
    def test_project_settings_tab_hidden_for_viewer(self):
        with patch.object(
            ...
            "as_user",
            return_value=self._project_conn(
                team_role="team-viewer", can_write=False, can_admin=False
            ),
        ):
            r = self.client.get(f"/projects/{self.pid}")
        assert r.status_code == 200
    ...
    def test_project_admin_settings_members_without_delete(self):
        ...
        with patch.object(
            ...
            "as_user",
            return_value=self._project_conn(
                tab="settings", team_role="team-member", can_write=True, can_admin=True
            ),
        ):
            r = self.client.get(f"/projects/{self.pid}?tab=settings")
        assert r.status_code == 200
    ...
    def test_create_secret_missing_key(self):
        r = self.client.post(
            f"/projects/{self.pid}/secrets", data={"key": "", "value": "x"}, follow_redirects=False
        )
        assert r.status_code == 302
    ...
    def test_delete_secret(self):
        ...
        with patch.object(db, "as_user", return_value=conn):
            r = self.client.post(
                f"/projects/{self.pid}/secrets/{sid}/delete", follow_redirects=False
            )
        assert r.status_code == 302
        conn.commit.assert_called()
    ...
    def test_reveal_secret(self):
        ...
        with patch.object(db, "as_user", return_value=conn):
            r = self.client.get(
                f"/projects/{self.pid}/secrets/{sid}/reveal", headers={"HX-Request": "true"}
            )
        assert r.status_code == 200
    ...
    def test_reveal_secret_requires_access_request(self):
        ...
        conn, cur = _conn()
        cur.fetchone.side_effect = [
            {"id": sid, "key": "API_KEY", "value_enc": enc, "expires_at": None},
            {"ok": True},
            {"a": False},
            {"r": True},
            None,
    ...
    def test_reveal_open_secret_no_approval(self):
        ...
        conn, cur = _conn()
        cur.fetchone.side_effect = [
            {"id": sid, "key": "FEATURE_FLAG", "expires_at": None},
            {"ok": True},
            {"a": False},
            {"r": False},
            {"value_enc": enc, "crypto_provider": "master"},
    ...
    def test_deny_secret_access(self):
        ...
        with patch.object(db, "as_user", return_value=conn):
            r = self.client.post(
                f"/projects/{self.pid}/access-requests/{rid}/deny", follow_redirects=False
            )
        assert r.status_code == 302
        assert "tab=requests" in r.location
        conn.commit.assert_called()
    ...
    def test_hide_secret(self):
        ...
        with self.client.session_transaction() as s:
            s["user_id"] = str(uuid4())
        r = self.client.get(
            f"/projects/{self.pid}/secrets/{sid}/hide", headers={"HX-Request": "true"}
        )
        assert r.status_code == 200
        assert b"*******" in r.data
