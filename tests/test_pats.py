...
class TestPatsUnit:
    ...
    def test_create_requires_expiry_when_policy_enabled(self):
        with (
            patch.object(pats, "count_for_user", return_value=0),
            patch.object(pats.settings_svc, "token_expiry_policy", return_value=(True, 3650)),
        ):
    ...
    def test_create_uses_policy_max(self):
        with (
            patch.object(pats, "count_for_user", return_value=0),
            patch.object(pats.settings_svc, "token_expiry_policy", return_value=(False, 30)),
        ):
...
class TestApiToken:
    ...
    def test_bad_pat_401(self):
        with patch.object(pats, "resolve", return_value=None):
            r = store.app.test_client().get(
                "/api/token", headers={"Authorization": "Bearer pat_invalid"}
            )
        assert r.status_code == 401
