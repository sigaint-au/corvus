...
class TestOrgAccess:
    ...
    def test_secret_acl_schema_and_config(self):
        ...
        assert (
            "deleted_at IS NOT NULL AND api.can_access_secret_row(id, project_id, access_mode, 'write', NULL)"
            in rbac
        )
    ...
    def test_eso_pat_bulk_export_filters_reveal_acl(self):
        ...
        assert not re.search(
            'SELECT key, value_enc FROM api\\.secrets\\s+WHERE project_id = %s AND deleted_at IS NULL\\s*\\"\\"\\"',
            body,
        )
    ...
    def test_org_groups_rbac_schema(self):
        ...
        assert (
            ...
            in Path(APP_ROOT / "integrations" / "ldap_auth.py").read_text()
        )
