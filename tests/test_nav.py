...
class TestNav:
    ...
    def test_select_team_leaves_other_team_project_secrets(self):
        ...
        with patch.object(nav, "_project_team_id", return_value=str(uuid4())):
            r = self.client.post(
                "/select-team",
                data={"team_id": str(self.tid), "next": f"/projects/{other_pid}?tab=secrets"},
                follow_redirects=False,
            )
        assert r.status_code == 302
    ...
    def test_redirect_after_team_switch_helper(self):
        ...
        with store.app.test_request_context("/"):
            with patch.object(nav, "_project_team_id", return_value="other"):
                assert (
                    nav.redirect_after_team_switch(f"/projects/{pid}?tab=secrets", "new")
                    == "/secrets"
                )
                assert (
                    nav.redirect_after_team_switch(f"/projects/{pid}?tab=settings", "new")
                    == "/projects"
                )
    ...
    def test_trash_empty_with_team(self):
        ...
        with (
            patch.object(db, "as_user", return_value=conn),
            patch.object(nav, "ensure_active_team", return_value=str(tid)),
        ):
            r = self.client.get("/trash")
        assert r.status_code == 200
    ...
    def test_restore_secret_denied(self):
        ...
        assert any(
            ("could not" in msg.lower() or "permission" in msg.lower() for _c, msg in flashes)
        )
