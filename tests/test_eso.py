...
class TestESO:
    ...
    def test_get_secret_service_read_forbidden(self):
        ...
        with patch.object(db, "connect", return_value=conn):
            r = self.client.get(
                f"/eso/v1/projects/{self.pid}/secrets/KEY",
                headers={"Authorization": "Bearer ss_read"},
            )
    ...
    def test_list_service_read_forbidden(self):
        ...
        with patch.object(db, "connect", return_value=conn):
            r = self.client.get(
                f"/eso/v1/projects/{self.pid}/secrets", headers={"Authorization": "Bearer ss_ro"}
            )
        assert r.status_code == 403
        assert "reveal" in r.get_json().get("error", "")
    ...
    def test_list_unauthorized(self):
        ...
        with patch.object(db, "connect", return_value=conn):
            r = self.client.get(
                f"/eso/v1/projects/{self.pid}/secrets", headers={"Authorization": "Bearer bad"}
            )
        assert r.status_code == 401
    ...
    def test_list_ok(self):
        ...
        with patch.object(db, "connect", return_value=conn):
            r = self.client.get(
                f"/eso/v1/projects/{self.pid}/secrets", headers={"Authorization": "Bearer ss_ok"}
            )
        assert r.status_code == 200
        ...
        assert any(
            any(isinstance(x, str) and "machine/values" in x for x in p) for p in audit_params
        )
    ...
    def test_delete_read_forbidden(self):
        ...
        with patch.object(db, "connect", return_value=conn):
            r = self.client.delete(
                f"/eso/v1/projects/{self.pid}/secrets/K", headers={"Authorization": "Bearer ss_ro"}
            )
        assert r.status_code == 403
    ...
    def test_mgmt_post_exempt_from_csrf_with_bearer(self):
        ...
        try:
            ...
            with (
                patch.object(pats, "resolve", return_value=uid),
    ...
    def test_upsert_no_auth(self):
        r = self.client.post(
            f"/eso/v1/projects/{self.pid}/secrets", json={"key": "K", "value": "v"}
        )
        assert r.status_code == 401
    ...
    def test_create_token_with_expiry(self):
        ...
        with (
            ...
            r = c.post(
                ...
                data={"name": "eso", "role": "reveal", "expires_days": "30"},
    ...
    def test_pat_list_projects(self):
        ...
        conn, cur = _conn(
            fetchall=[
                {"id": self.pid, "name": "ios-app", "team_id": uuid4(), "team_name": "Mobile"}
            ]
        )
        with (
            patch.object(pats, "resolve", return_value=uid),
    ...
    def test_machine_rejects_project_name(self):
        r = self.client.get(
            "/eso/v1/projects/ios-app/secrets/KEY", headers={"Authorization": "Bearer ss_x"}
        )
        assert r.status_code == 401
    ...
    def test_mgmt_create_token_requires_expiry_when_policy_enabled(self):
        ...
        with store.app.test_request_context("/x", method="POST", json={}):
            with (
                patch.object(mgmt_tokens, "_require_pat", return_value=(str(uuid4()), None)),
                patch.object(
                    mgmt_tokens.settings_svc, "token_expiry_policy", return_value=(True, 3650)
                ),
            ):
    ...
    def test_mgmt_upsert_meta_ok(self):
        ...
        ins = [
            c
            for c in cur.execute.call_args_list
            if c.args and "INSERT INTO api.secret_meta" in str(c.args[0])
        ]
    ...
    def test_mgmt_delete_meta_ok(self):
        ...
        dels = [
            c
            for c in cur.execute.call_args_list
            if c.args and "DELETE FROM api.secret_meta" in str(c.args[0])
        ]
    ...
    def test_parse_expires_at_capped(self):
        ...
        far = (
            (datetime.now(timezone.utc) + timedelta(days=config.MAX_EXPIRY_DAYS + 30))
            .date()
            .isoformat()
        )
    ...
    def test_mgmt_add_secret_binding_ok(self):
        ...
        ins = [
            c
            for c in cur.execute.call_args_list
            if c.args and "INSERT INTO rbac.bindings" in str(c.args[0])
        ]
    ...
    def test_mgmt_update_project_settings_ok(self):
        ...
        upd = [
            c
            for c in cur.execute.call_args_list
            if c.args and "UPDATE api.projects" in str(c.args[0])
        ]
    ...
    def test_mgmt_export_project_plain(self):
        ...
        cur.fetchall.return_value = [
            {"key": "A", "value_enc": enc, "note": "n", "crypto_provider": "master"}
        ]
        with (
            patch.object(pats, "resolve", return_value=uid),
            patch.object(db, "as_user", return_value=conn),
