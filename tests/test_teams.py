...
class TestTeams:
    ...
    def test_list_teams(self):
        tid = uuid4()
        conn, _ = _conn(
            fetchall=[{"id": tid, "name": "Platform", "role": "team-owner", "project_count": 2}]
        )
    ...
    def test_create_team(self):
        ...
        with (
            ...
            patch.object(settings_svc, "can_create_team", return_value=True),
        ):
    ...
    def test_user_directory_not_granted_to_authenticated(self):
        ...
        assert (
            ...
            in (REPO_ROOT / "db" / "migrations" / "0001_init.sql").read_text()
        )
    ...
    def test_tm_insert_policy_forbids_self_join(self):
        ...
        assert (
            ...
            not in rbac_sql.split("rbac_bindings_write")[1].split("CREATE POLICY")[0]
        )
    ...
    def test_add_member_viewer_role(self):
        ...
        with patch.object(db, "as_user", return_value=conn):
            r = self.client.post(
                f"/teams/{tid}/members",
                data={"email": "ro@ex.com", "role": "team-viewer"},
                follow_redirects=False,
            )
        assert r.status_code == 302
    ...
    def test_create_project(self):
        ...
        with patch.object(db, "as_user", return_value=conn):
            r = self.client.post(
                f"/teams/{tid}/projects", data={"name": "prod"}, follow_redirects=False
            )
        assert r.status_code == 302
