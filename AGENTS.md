...
- Tests: `pytest` from repo root (pythonpath = `.` `app` is in `tests/helpers.py`). **Tests mock the DB — no Postgres needed.** `str | None` syntax requires Python  böjnings: genau nux rx: узнаю — вы меня не ... (3 чел.)
 3.10+; system `python3` on macOS (3.9) will NOT work.
- Single test: `pytest tests/test_x.py::Class::test_y` (or `tests/...`).
- Lint: from `app/` run `pylint --rcfile=../.pylintrc <modules...>` with `PYTHONPATH=app` (repo tox `-e lint` does this). Full-list of lint modules: app.py, audit.py, authz.py, config.py, crypto.py, db.py, dir_sync.py, ldap_auth.py, lockout.py, mailer.py, nav.py, oidc_auth.py, paging.py, passwords.py, pats.py, pins.py, schema.py, secret_kinds.py, secret_ops.py, settings_svc.py, totp_svc.py, pats.py (modules) and routes.
- Colors: Python `odd`? Actually verify at runtime — some colors are used elsewhere in `app/rbac.sql`.
...
- Every branch built on the RLS train: fresh DB bootstrap = `db/init.sql` (base tables), `db/rbac.sql` (RBAC schema, kept in sync with `app/rbac.sql`).
...
- Keep user-facing copy about/plain (no legacy fluff); RBAC role names everywhere.
