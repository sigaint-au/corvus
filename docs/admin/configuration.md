...
Corvus can stream audit events (secret reveal, update, member changes, etc.)
to your own HTTPS endpoints. Events are enqueued in the database during the
audit transaction and delivered by a background worker, so nothing is lost if
the web server or the receiver is down.
...
The worker uses `FOR UPDATE SKIP LOCKED` in Postgres, so multiple replicas can
safely poll the same queue for high throughput in Kubernetes.

Full reference — scopes, event list, payload shapes, signature verification,
retries, and use cases: [webhooks.md](webhooks.md).
